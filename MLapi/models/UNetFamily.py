...
class Residual_block1(nn.Module):
    def __init__(self, ch_in, ch_out):
        ...
        self.conv_block = nn.Sequential(
            ...
            nn.BatchNorm2d(ch_out),
            nn.PReLU(),
            nn.Conv2d(ch_out, ch_out, kernel_size=1, stride=1, bias=False),
            nn.BatchNorm2d(ch_out),
            nn.PReLU(),
...
class Downstep(nn.Module):
    ...
    def forward(self, x):
        return torch.cat((self.conv_path(x), self.pool_path(x)), dim=1)
...
class RU_Net(nn.Module):
    def __init__(self, img_ch=3, output_ch=1):
        ...
        self.Conv1 = Residual_block(ch_in=img_ch, ch_out=64)
        self.Conv2 = Residual_block(ch_in=64, ch_out=128)
        self.Conv3 = Residual_block(ch_in=128, ch_out=256)
        self.aspp_bridge = ASPP(ch_in=256, ch_out=512)
        # self.Conv4 = Residual_block1(ch_in=256, ch_out=512)


        self.Up4 = up_conv(ch_in=512, ch_out=256)
        self.AAM4 = AAM(in_ch=256, out_ch=256)
        # self.Att4 = Attention_block(F_g=256, F_l=256, F_int=128)
        self.Up_conv4 = conv_block(ch_in=512, ch_out=256)

        self.Up3 = up_conv(ch_in=256, ch_out=128)
        self.AAM3 = AAM(in_ch=128, out_ch=128)
        # self.Att3 = Attention_block(F_g=128, F_l=128, F_int=64)
        self.Up_conv3 = conv_block(ch_in=256, ch_out=128)

        self.Up2 = up_conv(ch_in=128, ch_out=64)
        self.AAM2 = AAM(in_ch=64, out_ch=64)
        # self.Att2 = Attention_block(F_g=64, F_l=64, F_int=32)
        self.Up_conv2 = conv_block(ch_in=128, ch_out=64)
        self.aspp_out = ASPP(ch_in=128, ch_out=64)
...
class FCL_Net(nn.Module):
    def __init__(self, img_ch=3, output_ch=1, t=2):
        ...
        self.Conv1 = Residual_block(ch_in=img_ch, ch_out=64)
        self.Recurrent_block = Recurrent_block(ch_out=64)
        self.Conv2 = Residual_block(ch_in=64, ch_out=128)
        self.Recurrent_block = Recurrent_block(ch_out=128)
        self.Conv3 = Residual_block(ch_in=128, ch_out=256)
        self.aspp_bridge = ASPP(ch_in=256, ch_out=512)
        #self.Conv4 = Residual_block1(ch_in=256, ch_out=512)
