...
class Test():
    ...
    def save_segmentation_result(self):
        ...
        for i in range(self.test_imgs.shape[0]):
            ...
            imgRealName = img_name_list[i].split('/')[-1]
            save_img(total_img,join(self.save_img_path, "Result_"+imgRealName+'.png'))
            pngurl = str(os.getcwd())+'/' + 'experiments/UNet_vessel_seg/result_img' + '/'  +  "Result_"+imgRealName+'.png'
...
def finalTest():
    ...
    # net = models.UNetFamily.RU_Net(1, 2).to(device)
   # net = models.UNetFamily.FU_Net(1, 2, t=2).to(device)
    ...
    # net = models.U_Net(1, 2).to(device)
    #net = models.UNetFamily.FCL_Net(1, 2, t=2).to(device)
    #net = models.UNetFamily.rev_Net(1, 2, t=2).to(device)
    ...
    return eval.save_segmentation_result()
